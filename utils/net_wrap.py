...
def wrap_modules_in_net(net,cfg):
    wrapped_modules={}
    module_dict={}
    module_types = {"qkv":"qlinear_qkv", "proj":'qlinear_proj', 'fc1':'qlinear_MLP_1', 'fc2':"qlinear_MLP_2", 'linear':'qlinear_classifier','matmul1':"qmatmul_qk", 'matmul2':"qmatmul_scorev", "reduction": "qlinear_reduction"}
    ...
    for name,m in it:
        ...
        if isinstance(m,nn.Conv2d):
        ...
        elif isinstance(m,nn.Linear) and name[idx+1:] not in ['0', '1', '2']:
            # Linear Layer
            idx = idx+1 if idx != 0 else idx
            ...
            wrapped_modules[name] = new_m
            setattr(father_module,name[idx:],replace_m)
        if isinstance(m, Attention) and not cfg.no_LIS:
            m.forward = MethodType(attention_forward, m)
